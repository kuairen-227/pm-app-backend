...
public class EditCompletionCriterionHandlerTests : BaseCommandHandlerTest
{
    private readonly EditCompletionCriterionHandler _handler;
    private readonly Mock<ITicketRepository> _ticketRepository;
    private readonly TicketBuilder _ticketBuilder;

    public EditCompletionCriterionHandlerTests()
    {
        _ticketRepository = new Mock<ITicketRepository>();
        _ticketBuilder = new TicketBuilder();

        _handler = new EditCompletionCriterionHandler(
    ...
    public async Task 正常系_Handle()
    {
        ...
        // Act
        var command = new EditCompletionCriterionCommand(
            ticket.ProjectId, ticket.Id, ticket.CompletionCriteria[0].Id, "完了条件-編集済み"
        );
    ...
    public async Task 異常系_Handle_Ticketが存在しない場合()
    {
        // Arrange
        var command = new EditCompletionCriterionCommand(
            Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), "完了条件"
        );
