...
public class DeleteTicketHandlerTests : BaseCommandHandlerTest
{
    ...
    public async Task 正常系_Handle()
    {
        ...
        // Act
        var command = new DeleteTicketCommand(ticket.ProjectId, ticket.Id);
        var result = await _handler.Handle(command, CancellationToken.None);

        // Assert
        result.Should().Be(Unit.Value);
        _ticketRepository.Verify(x => x.DeleteAsync(ticket, It.IsAny<CancellationToken>()), Times.Once);
